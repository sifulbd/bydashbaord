...
export default function TrackingTableSort(props) {
            ...
            <Table className="bl-table" sx={{ minWidth: 650 }} stickyHeader aria-label="sticky table">
                <TableHead className="bl-table-head">
                    <TableRow className="tablerow">
                        ...
                        <TableCell onClick={() => requestSort("trackingNo")} className={`bl-tr-hide-xs ${getClassNamesFor("trackingNo")}`} align="right">
                            {" "}
                            <i className="fa fa-sort"></i> Tracking Number
                        </TableCell>
                        <TableCell onClick={() => requestSort("tracking")} className={`bl-tr-hide-xs ${getClassNamesFor("tracking")}`} align="right">
                            <i className="fa fa-sort"></i> Tracking
                        </TableCell>
                ...
                <TableBody>
                    {items.map((row) => (
                        <TableRow className="tablerow" key={row.name} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                            <TableCell>
                            ...
                            {row.tracking === "Delivered" ? (
                                <>
                                    ...
                                    <TableCell className="bl-tr-hide-xs" align="right">
                                        <div className="pod-content">
                                            Delivered <Button className="btn bl-btn bl-waiting-btn delivered"> {row.podButtonText}</Button>
                                        </div>
                                    </TableCell>
                                </>
                            ) : (
                                <>
                                    ...
                                    <TableCell className="bl-tr-hide-xs" align="right">
                                        <div className="pod-content">
                                            Waiting for Delivery <Button className="btn bl-btn bl-waiting-btn"> {row.podButtonText}</Button>
                                        </div>
                                    </TableCell>
                                </>
                            )}
                            ...
                            <TableCell align="right" className="bl-action-td">
                                ...
                                <div className="bl-table-dropdown">
                                    ...
                                    >
                                        <TrackingModal className="menuItem" text="Edit Tracking" />
                                        <Button onClick={() => alert("Are you sure")} className="menuItem" disableRipple>
                                            Delete Tracking
                                        </Button>
                                    </StyledMenu>
                                </div>
